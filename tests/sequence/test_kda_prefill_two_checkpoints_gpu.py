...
def dual_inputs(lengths, *, heads=1, offsets=None, seed=900, state_slots=16, device):
    count = len(lengths)
    inputs = make_inputs(
        lengths=lengths,
        heads=heads,
...
def assert_dual_oracle(binding, tensors, inputs):
    expected_out, expected_pool = run_oracle(inputs, max_checkpoints=2)
...
def test_two_checkpoint_gpu_independent_fp32_oracle(heads, inplace):
    ...
    assert_dual_oracle(binding, tensors, inputs)
...
def test_two_checkpoint_gpu_8k_matches_one_checkpoint_prefixes(heads):
    ...
    inputs = dual_inputs([8192], heads=heads, offsets=[[6144, 7168]], device=device)
    binding, tensors = make_binding(
        inputs,
        max_tokens=8192,
        max_seqs=1,
...
def test_two_checkpoint_gpu_frozen_replay_changes_live_counts_and_offsets():
    ...
    first = dual_inputs([96], device=device)
    binding, tensors = make_binding(
    ...
    try:
        ...
        for lengths, offsets in (
            ...
            assert_dual_oracle(binding, tensors, live)
...
def test_two_checkpoint_gpu_invalid_metadata_preserves_state(fault):
    ...
    inputs = dual_inputs([64], device=device)
    binding, tensors = make_binding(
...
def test_two_checkpoint_gpu_high_pool_offsets():
    ...
    inputs = dual_inputs([64], offsets=[[16, 48]], state_slots=4, device=device)
